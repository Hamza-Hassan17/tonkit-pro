...
<div class="bg-brand-darker text-gray-300 text-xs">
    <div class="container-site flex items-center justify-between h-9">
        <span class="hidden sm:inline uppercase tracking-widest text-[11px]">Custom Caps · Embroidery · DTF Print</span>
        <span class="hidden md:inline text-brand-orange font-medium">Shipping from Montreal, Canada</span>
        <a href="mailto:sales@capbeast.com" class="inline-flex items-center gap-1.5 hover:text-white">
            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75"/></svg>
            Questions? sales@capbeast.com
        </a>
    </div>
</div>
        ...
        <form action="{{ route('products.index') }}" method="GET" class="hidden md:flex flex-1 max-w-xl">
            <div class="relative w-full">
                ...
                <button type="submit" aria-label="Search"
                        class="absolute right-1.5 top-1.5 bottom-1.5 aspect-square rounded-full bg-brand-orange text-brand-dark flex items-center justify-center hover:bg-brand-orange-dark transition-colors">
        ...
        <div class="flex items-center gap-3 ml-auto">
            ...
            <a href="{{ route('cart.index') }}" class="relative flex items-center gap-2 group">
                <span class="relative h-9 w-9 rounded-full bg-gray-100 group-hover:bg-brand-orange/10 flex items-center justify-center transition-colors">
                    ...
                    @if ($cartCount > 0)
                        <span class="absolute -top-1 -right-1 bg-brand-dark text-white text-[10px] font-bold rounded-full h-4 min-w-4 px-1 flex items-center justify-center">{{ $cartCount }}</span>
                    @endif
                </span>
